...
import { Modal, Button, message, Select, Input, Icon, InputNumber, Spin } from 'antd';
...
class CashSaleFormSplit extends Component {
  ...
  state = {
    ...
    productCodeList: [null], //拆分的产品id数组
    fetching: false
  };
  ...
  render() {
    ...
    const formItems = [
      ...
      },
      {
        ...
        // options: allBondList.length > 0 ? allBondList : []
      },
      {
        name: 'settleCurrency',
        label: '结算币种',
            ...
            return (
              <SelectPage
                ...
                onChange={(e, option) => {
                  ...
                  let productCodeList = this.state.productCodeList;
                  productNameList[index] = option?.props?.origindata?.productName;
                  productIdList[index] = e;
                  productCodeList[index] = option?.props?.origindata?.productCode;
                  this.setState({ productNameList, productIdList, productCodeList });
                }}
              ></SelectPage>
            );
          }
    ...
    return (
      <>
        <Modal
          title={'债券回售--' + (openModalType == 'edit' ? '修改' : '新增')}
  ...
  async componentDidMount() {
    ...
    if (this.props.openModalType == 'edit') {
      this.setState({
        dataSource: this.props.ids[0].productList ? this.props.ids[0].productList : [{ id: 1 }]
      });
      // 设置拆分表的值
      let {
        faceValueList, //券面总额
      ...
      const productList = this?.props?.ids[0]?.productList;
      for (let n = 0; n < productList?.length; n++) {
        faceValueList[n] = productList[0].faceValue;
        totalAccruedInterestList[n] = productList[0].totalAccruedInterest;
        tradeAmountList[n] = productList[0].tradeAmount;
        settleAmountList[n] = productList[0].settleAmount;
        tradeFeeList[n] = productList[0].tradeFee;
        settleFeeList[n] = productList[0].settleFee;
        productNameList[n] = productList[0].productName;
        productIdList[n] = productList[0].productId;
        productCodeList[n] = productList[0].productCode;
        faceValueTotal += productList[0].faceValue;
        totalAccruedInterestTotal += productList[0].totalAccruedInterest;
        tradeAmountTotal += productList[0].tradeAmount;
        settleAmountTotal += productList[0].settleAmount;
        tradeFeeTotal += productList[0].tradeFee;
        settleFeeTotal += productList[0].settleFee;
      }
      this.setState({
        faceValueList,
  ...
  onSubmit = () => {
    ...
    form.validateFields(async (err, val) => {
      ...
      if (this.props.openModalType == 'add') {
        await asyncHttpPostAdd({ type: 'ZQHS', params });
      } else {
        await asyncHttpPostEdit({ type: 'ZQHS', params: { ...params, id: this.props.ids[0].id } });
      }
