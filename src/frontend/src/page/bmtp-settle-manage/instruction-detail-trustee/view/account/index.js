...
import { PageBody, $connect, Tabs, TransfromDown, urlHashToJson } from 'yss-biz';
...
class ClearanceCheck extends PureComponent {
  ...
  async componentDidMount() {
    ...
    await authModels.mutations
      .asyncHttpPermitVerity({ type: 'instruction-detail-trustee', functionCode, source })
      .then(() => {
  ...
  changeTab(activeKey) {
    const { asyncHttpSearchAccountByCondition, asyncHttpGetBondList } = this.props;
    let params = {
      ...page
    };
    if (activeKey == '1') {
...
export default $connect(ClearanceCheck, 'bmtp-settle-manage/instruction-detail-trustee');
