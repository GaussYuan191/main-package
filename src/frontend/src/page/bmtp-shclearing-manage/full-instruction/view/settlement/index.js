...
class PageComponent extends Component {
  ...
  async componentDidMount() {
    const { asyncHttpSqDebtList, asyncGetBusinessType, asyncHttpCurrentTradingDay, toResetSearch } =
      this.props;

    await authModels.mutations
      .asyncHttpPermitVerity({ type: 'full-instruction', functionCode, source })
      .then(() => {
    ...
    await asyncGetBusinessType(); // 获取业务类别字典数据, 上清所交易指令管理-新增 以及查询表单中用到

    // 初始化到期结算管理的交易日以及查询参数
    await asyncHttpCurrentTradingDay().then(() => {
      toResetSearch({
        type: 'fullInstruction',
        queryElement: {}
      });
    });
    await asyncHttpSqDebtList({});
    // await asyncHttpToGetCounterAccount({});
  }
...
export default $connect(PageComponent, 'bmtp-shclearing-manage/full-instruction');
