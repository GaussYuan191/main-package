...
class Settlement extends Component {
  ...
  async componentDidMount() {
    ...
    await authModels.mutations
      .asyncHttpPermitVerity({ type: 'settlement', functionCode, source })
      .then(() => {
        const btnAuth = authModels.getState().get('btnAuth').toJS();
        this.setState({ btnAuth });
      });

    await asyncHttpQueryZJDList({ type: 'instructManage' });
...
export default $connect(Settlement, 'bmtp-shclearing-manage/settlement');
