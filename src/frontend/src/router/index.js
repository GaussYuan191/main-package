...
routes.forEach(item => {
  let routeItem = null;
  try {
    routeItem = require(`../page/${item}/routes`).default;
    routerTree.push(...routeItem);
  } catch (error) {
    console.error(`路由配置中, ${item}页面不存在!`);
    return;
  }
});
...
const App = () => <Router forceRefresh={!supportsHistory}>{mapRoutes(routerTree)}</Router>;
export default App;
